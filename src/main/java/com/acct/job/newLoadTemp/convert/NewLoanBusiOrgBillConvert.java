...
public class NewLoanBusiOrgBillConvert {
     ...
     * @return
     */
    public static List<BusiOrgBillModel> makeBusiOrgBillModelList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<BusiOrgBillModel> branchList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            BusiOrgBillModel brchModeal = assembleBusiOrgBillTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
            if(brchModeal == null){
                continue;
            }
     ...
     * @return
     */
    public static BusiOrgBillModel assembleBusiOrgBillTemp(String lineStr,int readStartNum, int lineNum) {
...
//            busiOrgBillModel.setAcctDate(DateUtil.parseDate(buffers[0].toString(), "yyyyMMdd"));
//        }
