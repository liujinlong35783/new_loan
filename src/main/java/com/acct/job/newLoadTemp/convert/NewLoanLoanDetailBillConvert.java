...
public class NewLoanLoanDetailBillConvert {
     ...
     * @return
     */
    public static List<LoanDetailBillModel> makeLoanDetailBillList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<LoanDetailBillModel> loanDetailBillModelList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            LoanDetailBillModel loanDetailBillModel = assembleLoanDetailBillTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
     ...
     * @return
     */
    public static LoanDetailBillModel assembleLoanDetailBillTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        LoanDetailBillModel loanDetailBillModel = new LoanDetailBillModel();
