...
public class NewLoanInterestBillConvert {
     ...
     * @return
     */
    public static List<InterestBillLoanModel> makeInterestBillList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<InterestBillLoanModel> interestBillModelList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            InterestBillLoanModel interestBillModel = assembleInterestBillTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
     ...
     * @return
     */
    public static InterestBillLoanModel assembleInterestBillTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        InterestBillLoanModel interestBillModel = new InterestBillLoanModel();
...
//        }
        ...
        interestBillModel.setCreateDate(new Date());//预留创建时间字段
...
//        }
        ...
        interestBillModel.setChannel(buffers[22].toString());
        //新希望改
        interestBillModel.setPayoffOtherSum(buffers[23].toString());
        interestBillModel.setShouldOtherSum(buffers[24].toString());
