...
public class NewLoanAcctVoucherConvert {
     ...
     * @return
     */
    public static List<AccountVoucherLoanModel> makeAcctVoucherList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<AccountVoucherLoanModel> branchList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            AccountVoucherLoanModel voucherModel= assembleAcctVoucherTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
     ...
     * @return
     */
    public static AccountVoucherLoanModel assembleAcctVoucherTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        AccountVoucherLoanModel AccountVoucherModel = new AccountVoucherLoanModel();
...
//        }
        ...
        AccountVoucherModel.setCreateDate(new Date());
...
//        }
        ...
        AccountVoucherModel.setChannel(buffers[22].toString());
        //新希望改
        AccountVoucherModel.setAccountEntry(buffers[23].toString());
