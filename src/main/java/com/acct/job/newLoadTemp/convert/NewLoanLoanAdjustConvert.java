...
public class NewLoanLoanAdjustConvert {
     ...
     * @return
     */
    public static List<LoanAdjustLoanModel> makeLoanAdjustList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<LoanAdjustLoanModel> branchList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            LoanAdjustLoanModel brchModeal = assembleLoanAdjustTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
            if(brchModeal == null){
                continue;
            }
     ...
     * @return
     */
    public static LoanAdjustLoanModel assembleLoanAdjustTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        LoanAdjustLoanModel loanAdjustModel = new LoanAdjustLoanModel();
...
//        loanAdjustModel.setNewItemCode(buffers[12].toString());//预料创建时间
        ...
        loanAdjustModel.setChannel(buffers[14].toString());
        //新希望改
        loanAdjustModel.setOrgName(buffers[15].toString());
        loanAdjustModel.setOrgCode(buffers[16].toString());
        loanAdjustModel.setLoanAccount(buffers[17].toString());
        loanAdjustModel.setContractNo(buffers[18].toString());
