...
public class NewLoanBusiOrgSeqConvert {
     ...
     * @return
     */
    public static List<BusiOrgSeqLoanModel> makeBusiOrgSeqList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<BusiOrgSeqLoanModel> branchList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            BusiOrgSeqLoanModel brchModeal = assembleBusiOrgSeqTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
            if(brchModeal == null){
                continue;
            }
     ...
     * @return
     */
    public static BusiOrgSeqLoanModel assembleBusiOrgSeqTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        BusiOrgSeqLoanModel busiOrgSeqModel = new BusiOrgSeqLoanModel();
...
//        busiOrgSeqModel.setCreateDate(DateUtil.parseDate(buffers[10].toString(), "yyyy-MM-dd hh:mm:ss"));
        busiOrgSeqModel.setCreateDate(new Date());
        ...
        busiOrgSeqModel.setChannel(buffers[16].toString());
