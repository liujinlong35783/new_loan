...
public class NewLoanRefundReceiptConvert {
     ...
     * @return
     */
    public static List<RefundReceiptModel> makeRefundReceiptList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<RefundReceiptModel> refundReceiptModelList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            RefundReceiptModel refundReceiptModel = assembleRefundReceiptTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
     ...
     * @return
     */
    public static RefundReceiptModel assembleRefundReceiptTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        RefundReceiptModel refundReceiptModel = new RefundReceiptModel();
