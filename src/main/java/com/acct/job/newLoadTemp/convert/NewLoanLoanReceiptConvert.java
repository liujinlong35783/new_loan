...
public class NewLoanLoanReceiptConvert {
     ...
     * @return
     */
    public static List<LoanReceiptLoanModel> makeLoanReceiptList(String path, int readStartNum, int readEndNum){


        List<StringBuffer> lines = FileUtil.readFileNLine(path, readStartNum, readEndNum);
        List<LoanReceiptLoanModel> loanReceiptModelList = new ArrayList(HjFileFlagConstant.ONE_TIME_READ_LINE_NUM);
        for (int lineNum = 0; lineNum <lines.size(); lineNum++) {
            //行数据生成对象
            LoanReceiptLoanModel loanReceiptModel = assembleLoanReceiptTemp(lines.get(lineNum).toString(), readStartNum, lineNum);
     ...
     * @return
     */
    public static LoanReceiptLoanModel assembleLoanReceiptTemp(String lineStr,int readStartNum, int lineNum) {
        ...
        LoanReceiptLoanModel loanReceiptModel = new LoanReceiptLoanModel();
...
//        loanReceiptModel.setCreateDate(DateUtil.parseDate(buffers[14].toString(), "yyyy-MM-dd hh:mm:ss"));
        loanReceiptModel.setCreateDate(new Date());
        ...
        loanReceiptModel.setChannel(buffers[18].toString());
