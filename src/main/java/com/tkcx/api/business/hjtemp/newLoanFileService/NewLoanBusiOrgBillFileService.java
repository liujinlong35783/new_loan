...
import com.tkcx.api.business.acctPrint.service.Loan.BusiOrgBillLoanService;
...
public class NewLoanBusiOrgBillFileService {
    ...
    @Autowired
    private BusiOrgBillLoanService busiOrgBillLoanService;
    ...
    private void settleBusiOrgBillFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<BusiOrgBillLoanModel> busiOrgBillModelList = NewLoanBusiOrgBillConvert.makeBusiOrgBillModelList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            busiOrgBillLoanService.remove(null);
            log.info("BusiOrgBillModel数据清空成功");
        }
        busiOrgBillLoanService.saveBatch(busiOrgBillModelList);
        /** 更新已读取的文件信息 */
