...
public class NewLoanBusiOrgSeqFileService {
    ...
    @Autowired
    private BusiOrgSeqService busiOrgSeqService;
    ...
    private void settleBusiOrgSeqFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<BusiOrgSeqModel> busiOrgSeqModelList = NewLoanBusiOrgSeqConvert.makeBusiOrgSeqList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            busiOrgSeqService.remove(null);
            log.info("BusiOrgSeqModel数据清空成功");
        }
        busiOrgSeqService.saveBatch(busiOrgSeqModelList);
        /** 更新已读取的文件信息 */
