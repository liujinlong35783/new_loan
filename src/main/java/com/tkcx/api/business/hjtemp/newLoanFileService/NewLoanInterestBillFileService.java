...
public class NewLoanInterestBillFileService {
    ...
    private void settleInterestBillFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<InterestBillModel> interestBillModelList = NewLoanInterestBillConvert.makeInterestBillList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            interestBillService.remove(null);
            log.info("InterestBillModel数据清空成功");
        }
        interestBillService.saveBatch(interestBillModelList);
        /** 更新已读取的文件信息 */
