...
import com.tkcx.api.business.acctPrint.service.Loan.LoanAccBillLoanService;
import com.tkcx.api.business.acctPrint.service.Loan.LoanAccBillLoanTempService;
...
public class NewLoanLoanAccBillFileService {
    ...
    @Autowired
    private LoanAccBillLoanService loanAccBillLoanService;
    @Autowired
    private LoanAccBillLoanTempService loanAccBillLoanTempService;
    ...
    private void settleLoanAccBillFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<LoanAccBillLoanTempModel> loanAccBillModelList = NewLoanLoanAccBillConvert.makeLoanAccBillList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            loanAccBillLoanTempService.remove(null);
            log.info("AcctBrchTempModel数据清空成功");
        }
        loanAccBillLoanTempService.saveBatch(loanAccBillModelList);
        /** 更新已读取的文件信息 */
