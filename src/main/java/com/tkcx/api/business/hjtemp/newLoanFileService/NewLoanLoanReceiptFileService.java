...
import com.acct.job.newLoadTemp.convert.NewLoanLoanReceiptConvert;
...
import com.tkcx.api.business.acctPrint.service.LoanReceiptService;
...
@Slf4j
@Service
public class NewLoanLoanReceiptFileService {

    @Autowired
    private LoanReceiptLoanService loanReceiptLoanService;
     ...
     * @param queryResult
     */
    public void handleLoanReceiptFile(Boolean isRemove, HjFileInfoModel queryResult) {

        try{
            settleLoanReceiptFile(isRemove, queryResult);
        } catch (Exception e) {
            log.error("读取【{}】行到【{}】行异常：{}",
     ...
     * @param queryResult
     */
    private void settleLoanReceiptFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<LoanReceiptLoanModel> loanReceiptLoanList = NewLoanLoanReceiptConvert.makeLoanReceiptList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            loanReceiptLoanService.remove(null);
            log.info("LoanReceiptTempModel数据清空成功");
        }
        loanReceiptLoanService.saveBatch(loanReceiptLoanList);
        /** 更新已读取的文件信息 */
