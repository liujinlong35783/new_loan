...
public class NewLoanAccVoucherFileService {
    ...
    private void settleAccVoucherFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<AccountVoucherModel> accountVoucherModelList = NewLoanAcctVoucherConvert.makeAcctVoucherList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            accountVoucherService.remove(null);
            log.info("AccountVoucherModel数据清空成功");
        }
        accountVoucherService.saveBatch(accountVoucherModelList);
        /** 更新已读取的文件信息 */
