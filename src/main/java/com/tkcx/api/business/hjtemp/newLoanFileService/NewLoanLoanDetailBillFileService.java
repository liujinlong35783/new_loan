...
public class NewLoanLoanDetailBillFileService {
    ...
    @Autowired
    private LoanDetailBillService LoanDetailBillService;
    ...
    private void settleLoanDetailBillFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<LoanDetailBillModel> loanDetailBillModelList = NewLoanLoanDetailBillConvert.makeLoanDetailBillList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            LoanDetailBillService.remove(null);
            log.info("LoanDetailBillModel数据清空成功");
        }
        LoanDetailBillService.saveBatch(loanDetailBillModelList);
        /** 更新已读取的文件信息 */
