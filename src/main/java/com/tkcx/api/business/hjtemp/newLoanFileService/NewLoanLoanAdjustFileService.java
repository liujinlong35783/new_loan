...
public class NewLoanLoanAdjustFileService {
    ...
    @Autowired
    private LoanAdjustService loanAdjustService;
    ...
    private void settleLoanAdjustFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<LoanAdjustModel> loanAdjustModelList = NewLoanLoanAdjustConvert.makeLoanAdjustList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            loanAdjustService.remove(null);
            log.info("LoanAdjustModel数据清空成功");
        }
        loanAdjustService.saveBatch(loanAdjustModelList);
        /** 更新已读取的文件信息 */
