...
import com.tkcx.api.business.acctPrint.service.Loan.RefundReceiptLoanService;
...
public class NewLoanRefundReceiptFileService {
    ...
    @Autowired
    private RefundReceiptLoanService refundReceiptLoanService;
    ...
    private void settleRefundReceiptFile(Boolean isRemove, HjFileInfoModel queryResult) {
        ...
        List<RefundReceiptLoanModel> refundReceiptModelList = NewLoanRefundReceiptConvert.makeRefundReceiptList(filePath, readStartNum, readEndNum);
        /** 已读取完成的数据入库 */
        if(isRemove) {
            refundReceiptLoanService.remove(null);
            log.info("AcctBrchTempModel数据清空成功");
        }
        refundReceiptLoanService.saveBatch(refundReceiptModelList);
        /** 更新已读取的文件信息 */
